...
public enum TestTableViewSort {
  ...
  TEST(
      "Test",
      Comparator.comparing(testTableView -> testTableView.getTest().getName()),
      Comparator.comparing(testTableView -> testTableView.test().name())),
  ASSAY(
      "Assay",
      Comparator.comparing(testTableView -> testTableView.getAssayName()),
      Comparator.comparing(ExternalTestTableView::assayName)),
  DONOR(
      "Donor",
      Comparator.comparing(testTableView -> testTableView.getDonor().getName()),
      Comparator.comparing(testTableView -> testTableView.donor().getName())),
  LAST_ACTIVITY(
      "Latest Activity",
      Comparator.comparing(TestTableView::getLatestActivityDate),
      Comparator.comparing(ExternalTestTableView::latestActivityDate));
  // @formatter:on

  private static final Map<String, TestTableViewSort> map =
      Stream.of(TestTableViewSort.values())
