...
public class ProjectRestController {
  ...
  public TableData<ProjectSummary> query(@RequestBody DataQuery query) {
    ...
    return caseService.getProjects(
  ...
  public TableData<OmittedRunSample> getSequencingOmissions(
      String projectName, DataQuery query, MetricCategory sequencingType) {
    OmittedRunSampleSort sort = parseSort(query, OmittedRunSampleSort::getByLabel);
    boolean descending = parseDescending(query);
    return caseService.getOmittedRunSamplesForProject(
        projectName, sequencingType, query.getPageSize(), query.getPageNumber(), sort, descending);
  }
}
