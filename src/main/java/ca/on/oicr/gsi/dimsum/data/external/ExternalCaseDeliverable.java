...
public record ExternalCaseDeliverable(
    String deliverableCategory,
    AnalysisReviewQcStatus analysisReviewQcStatus,
    LocalDate analysisReviewQcDate,
    ReleaseApprovalQcStatus releaseApprovalQcStatus,
    LocalDate releaseApprovalQcDate,
    List<ExternalCaseRelease> releases) {

  public ExternalCaseDeliverable(CaseDeliverable from) {
    this(
        from.getDeliverableCategory(),
        from.getAnalysisReviewQcStatus(),
        from.getAnalysisReviewQcDate(),
        from.getReleaseApprovalQcStatus(),
        from.getReleaseApprovalQcDate(),
        from.getReleases().stream()
            .map(ExternalCaseRelease::new)
