...
import { makeCopyNameButton, getRequiredElementById, makeCopyIdButton } from "./util/html-utils";
...
function reload(definition: TableDefinition<any, any>) {
  ...
  new TableBuilder(definition, tableContainerId, getSearchParams(), updateUrlParams).build();
}
...
switch (tableContainer.getAttribute("data-detail-type")) {
  ...
  case "CASE_ID": {
    ...
    if (internalUser) {
      ...
      button.onclick = (event) => (window.location.href = urls.dimsum.caseQcReport(caseId));
